...
class JourneyManager {
    constructor() {
        ...
        this.debugMode = true;
        this.resizeObservers = new Set(); // Track observers for cleanup
        
        // Only setup form listeners - tab switching handled by AdminUI
        this.setupFormListeners();
        
        console.log('✅ JourneyManager created and ready for integration');
    }

    // REMOVED: initializeEventListeners() - no longer needed with AdminUI integration
    ...
    // Enhanced Journey Tab Initialization
    async initializeJourneyTab() {
        if (this.map && this.journeys.length > 0) {
            // If already initialized, just force a resize and re-render
        ...
        try {
            ...
            // Wait a bit for the tab to be fully visible before initializing map
            setTimeout(() => {
                this.initializeMap();
                this.renderJourneyList();
    ...
    setupMapResizeObserver() {
        ...
        // Observe both the map container and its parent for maximum compatibility
        ...
        containers.forEach((container, index) => {
            if (container && !container.journeyResizeObserver) {
                ...
                container.journeyResizeObserver = observer; // Store reference to avoid duplicates
                this.resizeObservers.add(observer); // Track for cleanup
    ...
    updateOverviewStats() {
        ...
        // Notify admin core of data change for other stats
    ...
    showEditor() {
        ...
        if (editor) {
            ...
            // Force map resize when editor becomes visible
            setTimeout(() => this.forceMapResize(), 100);
        }
    }
    ...
    renderJourneyOnMap() {
        ...
        try {
            ...
            journey.bindPopup(this.escapeHtml(this.currentJourney.name));
    ...
    renderSegmentList() {
        ...
        listContainer.innerHTML = this.currentJourney.segments.map((segment, index) => `
            <div class="segment-item">
                ...
                <div class="segment-description">${this.escapeHtml(segment.description) || 'No description'}</div>
...
// REMOVED: DOMContentLoaded listener - now handled by AdminUI system
// Journey Manager is created by AdminUI when needed

// Add CSS animation for notifications (only if not already added)
if (!document.getElementById('journey-notification-styles')) {
    ...
    notificationStyleSheet.id = 'journey-notification-styles';

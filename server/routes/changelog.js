...
router.get('/', async (req, res) => {
  try {
    ...
    const changelog = commits.map(commit => {
      ...
      return {
        ...activity,
        // Use the review's itemName (proper name) instead of the parsed commit itemName (might be ID)
        itemName: review.itemName || activity.itemName,
        review: review,
...
router.post('/submit-review', requireAuth, async (req, res) => {
  try {
    ...
    if (userRole === 'gm' && user === 'gm') {
      ...
      return res.json({ 
        success: true, 
        message: 'GM self-action - no review needed',
        skipped: true 
      });
    }
...
// Manual cleanup of old approved reviews (GM/Admin only)
router.post('/cleanup-reviews', requireAuth, async (req, res) => {
  try {
    const userRole = req.session.role || 'user';
    const user = req.session.displayName || req.session.username || 'Unknown';

    // Check if user has permission to perform cleanup
    if (userRole !== 'gm' && userRole !== 'admin') {
      return res.status(403).json({ 
        success: false, 
        error: 'Only GM/Admin can perform review cleanup' 
      });
    }
    ...
    // Allow custom retention period from request body (default 30 days)
    ...
    // Filter out old approved/rejected reviews
    ...
    reviewsData.reviews = reviewsData.reviews.filter(review => {
      ...
      // For approved reviews, only remove if older than cutoff
      const reviewDate = new Date(review.reviewedAt || review.timestamp);
      if (reviewDate <= cleanupCutoff) {
        removedReviews.push({
    ...
    // Save updated reviews if any were removed
    if (removedCount > 0) {
      await saveReviews(reviewsData);
      console.log(`🧹 Manual cleanup by ${user}: Removed ${removedCount} old reviews (retention: ${retentionDays} days)`);
    }

    res.json({ 
      success: true,
      message: `Cleanup completed: ${removedCount} old reviews removed`,
      details: {
        beforeCount: beforeCleanup,
        afterCount: afterCleanup,
        removedCount: removedCount,
        retentionDays: retentionDays,
        cutoffDate: cleanupCutoff.toISOString(),
        removedReviews: removedReviews
      }
    });
  } catch (error) {
    console.error('Error during manual review cleanup:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
